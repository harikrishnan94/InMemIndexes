...
#define BTREE_UPDATE_STAT(stat)                            \
	do                                                     \
	{                                                      \
		if constexpr (Traits::STAT)                        \
		{                                                  \
			this->m_stats.CONCAT(CONCAT(num_, stat), s)++; \
		}                                                  \
	} while (0)
...
#define INNER_ONLY \
	template <typename Dummy = void, typename = typename std::enable_if_t<IsInner(), Dummy>>
#define LEAF_ONLY \
...
private:
	...
	struct node_t
	{
		...
		canTrim() const
		{
			return this->num_dead_values > 1;
		}

		inline bool
		canSplit() const
		{
			return this->num_values > 2;
		}
	...
	struct alignas(std::pair<Key, ValueType>) inherited_node_t : node_t
	{
		...
		int
		upper_bound_pos(const Key &key) const
		{
			int firstslot = IsLeaf() ? 0 : 1;
			int *slots    = get_slots();
			int pos =
		...
		inline void
		update_meta_after_insert(int datasize)
		{
			this->num_values++;
		...
		inline InsertStatus
		insert_into_pos(const Key &key, int pos)
		{
		...
		INNER_ONLY
		inline void
		update(int pos, const value_t &val)
		{
			value_t *oldval = pos ? &get_key_value(pos)->second
		...
		lower_bound(const Key &key) const
		{
			...
			bool present = pos < this->num_values && !key_greater(get_key_value(pos)->first, key);
		...
		std::pair<InsertStatus, int>
		insert(const Key &key, int pos = -1)
		{
		...
		NodeSplitInfo<inherited_node_t>
		split() const
		{
	...
	template <typename Node>
	inline void
	create_root(NodeSplitInfo<Node> splitinfo)
	{
		inner_node_t *new_root =
		    inner_node_t::alloc(splitinfo.left->lowkey, splitinfo.right->highkey, ++m_height);
	...
	get_merge_info(const Node *node, const inner_node_t *parent) const
	{
		if (parent == nullptr)
			return std::nullopt;
	...
	template <typename Node>
	void
	remove_from_node(Node *node, int pos)
	{
		node->remove(pos);
	...
	split_node(Node *node, inner_node_t *parent)
	{
		...
		if (node != m_root)
		{
			...
			std::tie(parent, right_pos) = insert_into_node(parent, splitinfo.split_key, right_pos);
	...
	template <typename Node>
	Node *
	trim_node(Node *node, inner_node_t *parent)
	{
	...
	template <typename Node>
	Node *
	handle_overflow(Node *node, const Key &key)
	{
		inner_node_t *parent = find_parent(node);
	...
	template <typename Node>
	std::pair<Node *, int>
	insert_into_node(Node *node, const Key &key, int pos = -1)
	{
		InsertStatus status;
	...
	inline leaf_node_t *
	get_prev_leaf(const leaf_node_t *leaf) const
	{
		node_t *current;
	...
	inline leaf_node_t *
	get_next_leaf(const leaf_node_t *leaf) const
	{
...
public:
	...
	public:
		...
		template <IteratorType OtherIType, typename OtherMapType, typename OtherLeafType>
		inline iterator_impl(const iterator_impl<OtherIType, OtherMapType, OtherLeafType> &it)
	...
	using const_iterator   = iterator_impl<IteratorType::FORWARD, const map, const leaf_node_t>;
	using reverse_iterator = iterator_impl<IteratorType::REVERSE, map, leaf_node_t>;
