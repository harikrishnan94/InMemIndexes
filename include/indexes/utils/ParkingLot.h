...
namespace detail
{
	struct WaitNodeBase
	{
		...
		WaitNodeBase(uint64_t key, uint64_t lotid) : m_key(key), m_lotid(lotid), m_signaled(false)
		{}
...
public:
	...
	ParkResult
	park(const Key key, D &&data, ToPark &&toPark, PreWait &&preWait)
	{
...
template <typename Key,
          ...
          typename Duration>
ParkResult
ParkingLot<Data>::park_until(const Key bits,
                             D &&data,
                             ToPark &&toPark,
                             PreWait &&preWait,
                             std::chrono::time_point<Clock, Duration> deadline)
...
void
ParkingLot<Data>::unpark(const Key bits, Func &&func)
{
	auto key     = std::hash<Key>{}(bits);
	auto &bucket = parking_lot::detail::Bucket::bucketFor(key);
