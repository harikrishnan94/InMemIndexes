...
private:
  ...
  struct inherited_node_t : node_t {
    ...
    inline key_value_t *get_key_value(int slot) const {
      ...
      return get_key_value_for_offset(
    ...
    // Must be called with this's mutex held
    inline InsertStatus insert_into_pos(const Key &key, const value_t &val,
                                        int pos) {
    ...
    // Must be called with this's mutex held
    InsertStatus insert(const Key &key, const value_t &val) {
    ...
    // Must be called with this's mutex held
    LEAF_ONLY
    std::pair<InsertStatus, std::optional<value_t>> upsert(const Key &key,
                                                           const Value &val) {
      bool key_present = false;
      int pos = 0;
    ...
    // Must be called with this's mutex held
    LEAF_ONLY
    inline std::optional<value_t> update_leaf(const Key &key,
    ...
    // Must be called with this's mutex held
    INNER_ONLY inline void update_inner_for_trim(const Key &key,
                                                 value_t child) {
      int pos = search_inner(key);
      ...
      atomic_node_update(
          [&]() { oldchild->store(child, std::memory_order_release); });
    }

    // Must be called with this's mutex held
    INNER_ONLY
    inline InsertStatus update_inner_for_split(const NodeSplitInfo &splitinfo) {
      ...
      if (this->haveEnoughSpace()) {
        ...
        new (this->opaque() + current_value_offset)
            key_value_t{split_key, right_child};
    ...
    // Must be called with this's mutex held
    INNER_ONLY
    inline void update_inner_for_merge(int merged_pos, value_t merged_child) {
      std::atomic<int> *slots = this->get_slots();
    ...
    LEAF_ONLY
    void get_slots_less_than(const Key &key,
                             std::vector<int> &slot_offsets) const {
      std::atomic<int> *slots = this->get_slots();
      auto [pos, found] = lower_bound(key);
  ...
  OpResult traverse(const GetChild &get_children, SnapshotVectorType &nss_vec,
  ...
  template <bool FillSnapshotVector, typename GetChild, int MaxRestarts = 2,
            ...
            typename NodeSnaphotType =
                std::conditional_t<FillSnapshotVector, DummyType, NodeSnapshot>>
  bool traverse_to_leaf(GetChild &&get_child, SnapshotVectorType &nss_vec,
                        NodeSnaphotType &leaf_snapshot) const {
    int restart_count = 0;
    ...
    do {
      opt_traversal_res = traverse<OPTIMISTIC_LOCKING, FillSnapshotVector>(
          get_child, nss_vec, leaf_snapshot);
      restart_count++;
    } while (opt_traversal_res != OpResult::SUCCESS &&
  ...
  bool get_leaf_containing(const Key &key, NodeSnapshotVector &nss_vec) const {
    DummyType dummy;

    bool is_leaf_locked = traverse_to_leaf<FILL_SNAPSHOT_VECTOR>(
        [&key](node_t *current) {
          return ASINNER(current)->get_child_for_key(key);
        },
        nss_vec, dummy);
  ...
  inline void store_root(node_t *new_root) {
    m_root_state.store(m_root_state.load().set_locked(),
  ...
  inline void create_root(NodeSplitInfo splitinfo) {
    inner_node_t *new_root = inner_node_t::alloc(
        splitinfo.left->lowkey, splitinfo.right->highkey, m_height + 1);
  ...
  insert_into_splitnode(const NodeSplitInfo &parent_splitinfo,
    ...
    inner_node_t *parent = ASINNER(
        key_less(*child_splitinfo.split_key, *parent_splitinfo.split_key)
  ...
  OpResult replace_subtree_on_version_match(const NodeSnapshotVector &nss_vec,
                                            int from_ss, Update &&update) {
    static thread_local std::vector<node_t *> deleted_nodes;

    auto res = [&]() {
      std::vector<std::unique_lock<indexes::utils::Mutex>> locks;
      for (int node_ss = from_ss; node_ss < static_cast<int>(nss_vec.size());
           node_ss++) {
  ...
  split_node(int node_ss, const NodeSnapshotVector &nss_vec,
  ...
  template <typename Node>
  OpResult trim_node(int node_ss, const Key &key,
  ...
  std::pair<OpResult, NodeSplitInfo>
  handle_node_overflow(int node_ss, const Key &key,
                       const NodeSnapshotVector &nss_vec,
                       NodeSplitInfo &prev_split_info) {
  ...
  void handle_overflow(const NodeSnapshotVector &nss_vec, const Key &key) {
    ...
    BTREE_DEBUG_ASSERT(nss_vec[node_ss].node &&
                       nss_vec[node_ss].node->isLeaf());

    while (node_ss > 0) {
      auto res = handle_node_overflow(node_ss, key, nss_vec, top_splitinfo);

      if (res.first == OpResult::STALE_SNAPSHOT ||
          res.first == OpResult::SUCCESS) {
  ...
  template <bool DoUpsert, typename OutputType = std::conditional_t<
                               DoUpsert, std::optional<Value>, bool>>
  inline std::pair<OpResult, OutputType>
  insert_or_upsert_leaf(const NodeSnapshotVector &nss_vec, bool is_leaf_locked,
  ...
  update_leaf(const NodeSnapshot &leaf_snapshot, const Key &key,
              const Value &val) {
    leaf_node_t *leaf = ASLEAF(leaf_snapshot.node);
    MutexLockType lock{leaf->mutex};
  ...
  auto insert_or_upsert(const Key &key, const Value &val) {
    ...
    while (true) {
      ...
      if (auto res = insert_or_upsert_leaf<DoUpsert>(nss_vec, is_leaf_locked,
                                                     key, val);
  ...
  template <typename Node>
  void merge_node(int node_ss, const NodeSnapshotVector &nss_vec,
                  const Key &key) {
    if (node_ss == 1)
      return;
  ...
  delete_from_leaf(const Key &key, bool is_leaf_locked,
  ...
  inline NodeSnapshot get_prev_leaf_containing(const Key &key) const {
    NodeSnapshot leaf_snapshot{};
    DummyType dummy;

    bool is_leaf_locked = traverse_to_leaf<NO_FILL_SNAPSHOT_VECTOR>(
        [&](node_t *current) {
          return ASINNER(current)->get_value_lower_than(key);
        },
        dummy, leaf_snapshot);
  ...
  leaf_node_t *get_prev_leaf(const Key &lowkey, std::vector<int> &slots) const {
    leaf_node_t *leaf;
...
public:
  ...
  std::optional<Value> Upsert(const Key &key, const Value &val) {
    return insert_or_upsert<DO_UPSERT>(key, val);
  }

  std::optional<Value> Update(const Key &key, const Value &val) {
    while (true) {
      EpochGuard eg(this);
      NodeSnapshot leaf_snapshot = get_leaf_containing(key);
    ...
    void increment() {
      if (++m_curpos >= static_cast<int>(m_slots.size())) {
        if (this->m_leaf->highkey) {
          m_leaf = ASLEAF(
              m_bt->get_next_leaf(this->m_leaf->highkey.value(), m_slots));
          m_curpos = 0;
        } else {
          m_leaf = nullptr;
          m_curpos = 0;
    ...
    void decrement() {
      if (--m_curpos < 0) {
        if (this->m_leaf->lowkey) {
          m_leaf = ASLEAF(
              m_bt->get_prev_leaf(this->m_leaf->lowkey.value(), m_slots));
  ...
  public:
    inline iterator_impl(const concurrent_map *bt, const leaf_node_t *leaf,
  ...
  inline const_reverse_iterator crbegin() const {
    ...
    return leaf ? const_reverse_iterator{this, leaf, std::move(slots),
