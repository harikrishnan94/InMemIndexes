...
static constexpr bool SYNCHRONIZE   = true;
static constexpr bool NOSYNCHRONIZE = false;

template <bool ScanSupported, bool SynchronizationNeeded, template <typename...> class MapType>
...
public:
	     ...
	     std::vector<KVPair> &result)
	{
		...
		if constexpr (SynchronizationNeeded)
		{
	...
	Insert(const std::string &table, const std::string &key, std::vector<KVPair> &values)
	{
		if constexpr (SynchronizationNeeded)
			...
			return insert(table + key, values, true);
	}
...
private:
	...
	int
	insert(const std::string &key, Cont &values, bool locked)
	{
