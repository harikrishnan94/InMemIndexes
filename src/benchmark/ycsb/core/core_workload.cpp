...
void CoreWorkload::BuildValues(ycsbc::DB::FieldVec &values) {
...
void CoreWorkload::BuildValues(ycsbc::DB::FieldMap &values) {
  for (int i = 0; i < field_count_; ++i) {
...
void CoreWorkload::BuildUpdate(ycsbc::DB::FieldMap &update) {
  update[NextFieldName()] =
      std::string(field_len_generator_->Next(), utils::RandomPrintChar());
}
