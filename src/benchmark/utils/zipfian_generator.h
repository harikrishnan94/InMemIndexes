...
public:
	...
	uint64_t NextUnlocked(uint64_t num_items);

	uint64_t
	NextUnlocked()
	{
		return NextUnlocked(num_items_);
	}
...
private:
	...
	///
	static double
	Zeta(uint64_t last_num, uint64_t cur_num, double theta, double last_zeta)
	{
	...
	static double
	Zeta(uint64_t num, double theta)
	{
...
inline uint64_t
ZipfianGenerator::NextUnlocked(uint64_t num)
{
...
inline uint64_t
ZipfianGenerator::Next(uint64_t num)
{
	std::lock_guard<std::mutex> lock(mutex_);

	return NextUnlocked(num);
}

inline uint64_t
ZipfianGenerator::LastUnlocked()
{
	return last_value_;
}

inline uint64_t
ZipfianGenerator::Last()
{
	std::lock_guard<std::mutex> lock(mutex_);
	return LastUnlocked();
}
