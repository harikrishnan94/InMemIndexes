...
TEST_CASE("BtreeMixedTest", "[btree]")
{
	...
	auto get_void_ptr = [](long v)
						{
							return reinterpret_cast<const void *>(v);
						};
	auto convert_to_void_ptr = [](long *v)
							   {
	...
	std::unordered_map<int, long> key_values;
	...
	for (int i = 0; i < num_operations; i++)
	{
		...
		switch (op)
		{
			case INSERT_OP:
			{
				...
				{
					REQUIRE(btree_find(btree, get_key_ptr(&key),
									   convert_to_void_ptr(&val)) == true);
					REQUIRE(btree_delete(btree, get_key_ptr(&key),
										 convert_to_void_ptr(&val)) == true);
					REQUIRE(val == key_values[key]);
					key_values.erase(key);
				}

				val = key * key_dist(e2);

				REQUIRE(btree_insert(btree, get_key_ptr(&key), get_void_ptr(val)) == true);
